...
export const p_home = () => {
    ...
    const itemsTemplate = (item) => {
        return node.div()
            .setClass('flex col-span-3 md:col-span-1 p-2 mt-1 mr-1 border-b-1 border-[darkgreen] hover:border-[orange] rounded-sm bg-[#00000099]')
            .setChildren([
                node.div().setText('🥇'),
    ...
    let jsdom = node.divimg('', 'swril.jpg').setClass('h-[600px] flex items-end pb-[30px]').setStyle({ minHeight: 'calc(100vh - 102px)' }).setChildren([
        node.div().setClass('bg-[#00000099] backdrop-blur-sm p-5 h-[450px] overflow-hidden').setChildren([
            ...
            node.scroller('', 'calc(100vw - 30px)', '250px', 'springgreen', '#333', '15px', ui.layoutH(node.vm_list('features', itemsTemplate, itemsData), 3))
        ]),
    ])
